...
class Player(pygame.sprite.Sprite):
    ...
    def cal_distance(self, x1, y1, x2, y2):
        return ((abs(x1 - x2) ** 2 + abs(y1 - y2) ** 2) ** 0.5) / 776

    def create_nn_input(self, screen_width, screen_height, obstacles, player_x, player_y, output_length):
        l, c, r = None, None, None
        for o in range(obstacles.__len__()):
            obs = obstacles[o]
            if obs['x'] == 177:
                if l is None: l = obs
            elif obs['x'] == 410:
            ...
            if r is not None and l is not None and c is not None:
                break

        l = 0 if l is None else 1 - self.cal_distance(player_x, player_y, l['x'], l['y'])
        r = 0 if r is None else 1 - self.cal_distance(player_x, player_y, r['x'], r['y'])
        c = 0 if c is None else 1 - self.cal_distance(player_x, player_y, c['x'], c['y'])
        # l = 0 if l is None else 1 - ((player_y - l['y']) / 740)
        # r = 0 if r is None else 1 - ((player_y - r['y']) / 740)
        # c = 0 if c is None else 1 - ((player_y - c['y']) / 740)

        ret = np.asarray([
            l, # left obs distance
            c, # center obs distance
            r, # right obs distance
        ])
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        nn_input = self.create_nn_input(screen_width, screen_height, obstacles, player_x, player_y, self.nn.input_layer_size)
        result = self.nn.forward(nn_input)
